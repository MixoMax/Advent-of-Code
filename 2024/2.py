...
res = [is_report_safe([int(x) for x in data[idx//2].split()]) if idx % 2 == 0 else is_report_safe_with_dampener([int(x) for x in data[idx//2].split()]) for idx in range(len(data)*2)]
